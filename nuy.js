...
const nuy = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('QR BY.','white'),color('Nurulツ','red'),color('X','lime'),color('FxSx','red'))
})
...
✿➤ Ketik : ${prefix}daftar`
				...
				teks = `*Sayonara* @${num.split('@')[0]}👋\n*Nitip Gorengan Buat Buka Puasa*🤭`
				let buff = await getBuffer(ppimg)
				nuy.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	nuy.on('message-new', async (mek) => {
		try {
		    ...
		    nuy.sendMessage(from, teks, image, {quoted:mek})
		    }
        ...
        if (levelRole <= 2) {
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("JAGA UCAPAN DONG!! 😠")
                        .then(() => nuy.groupRemove(from, sender))
                        .then(() => {
                            nuy.sendMessage(from, `*「 ANTI BADWORD 」*\nManca-Bot akan kick kamu karena berkata kasar!`, text ,{quoted: mek})
                        }).catch(() => nuy.sendMessage(from, `Untung cya bukan admin, kalo admin udah cya kick!`, text , {quoted : mek}))
                } else {
          ...
          const checkLimit = (sender) => {
          	let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            nuy.sendMessage(from, ind.limitcount(limitCounts), text, { quoted : mek})
                            found = true
                        }
                    }
                    if (found === false) {
           ...
           const isLimit = (sender) =>{ 
              ...
              if (limits >= limitawal ) {
              	  position = true
                    nuy.sendMessage(from, ind.limitend(pushname), text, {quoted: mek})
                    return true
              } else {
              	_limit
                  position = true
...
//group message
			...
			switch(command) {
				case 'antibadword':  // Fix Bug By MancaBot & Nurul*
            ...
            case 'gimg':
            case 'getimg':
            iif (!isRegistered) return reply( ind.noregis())
            ...
            case 'addvid':
             ...
             case 'gvid':
                ...
                if (!isRegistered) return reply( ind.noregis())
                    query = args.join(" ")
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/kusonimesearch?apikey=682aeab645ed61cf137cf971&query=${query}`)
                    ...
                    nuy.sendMessage(from, buffer, image, { quoted: mek, caption: txt })
                    break
				case 'shadow':
                case 'cup':
                case 'cup1':
                ...
                case 'carvedwood':  // Fix Bug By MancaBot & Nurul*
                    ...
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/photooxy1/${command}?apikey=682aeab645ed61cf137cf971&text=${txt}`)
		...
		case 'transfer':
				...
				nuy.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": adm}})
				break
                case 'premium':
				if (!isOwner) return reply(ind.ownerb())
				premm = body.slice(9)
		...
		case 'unpremium':
				...
				nuy.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": prem}})
				break
         case 'ban':
				if (!isOwner) return reply(ind.ownerb())
				bnnd = body.slice(5)
		...
		case 'unban':
				...
				nuy.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": ban}})
				break
		...
		case 'giftlimit': 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				data = await fetchJson(`https://tobz-api.herokuapp.com/api/moddroid?q=${body.slice(10)}&apikey=Bidmzz1sJ2L1TKyqaMEU`)
				hepi = data.result[0] 
		...
		case 'happymod':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				data = await fetchJson(`https://tobz-api.herokuapp.com/api/happymod?q=${body.slice(10)}&apikey=Bidmzz1sJ2L1TKyqaMEU`)
				hupo = data.result[0] 
		...
		case 'bitly':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				nuy.updatePresence(from, Presence.composing) 
				data = await fetchJson(`https://tobz-api.herokuapp.com/api/bitly?url=${args[0]}&apikey=Bidmzz1sJ2L1TKyqaMEU`)
				hasil = `link : ${args[0]}\n\nOutput : ${data.result}`
				reply(hasil)
				await limitAdd(sender)
				break
		...
		case 'shrtco':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				nuy.updatePresence(from, Presence.composing) 
				data = await fetchJson(`http://lolhuman.herokuapp.com/api/shortlink2?url=${args[0]}&apikey=682aeab645ed61cf137cf971`)
				hasil = `link : ${args[0]}\n\nOutput : ${data.result}`
				reply(hasil)
				await limitAdd(sender)
				break
		...
		case 'galaxtext':
				...
				ct = body.slice(10)
				reply(ind.wait())
				ct = await getBuffer(`http://lolhuman.herokuapp.com/api/ephoto1/galaxywallpaper?apikey=682aeab645ed61cf137cf971&text=${ct}`)
				nuy.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'pupycut':
				...
				ct = body.slice(8)
				reply(ind.wait())
				ct = await getBuffer(`http://lolhuman.herokuapp.com/api/ephoto1/puppycute?apikey=682aeab645ed61cf137cf971&text=${ct}`)
				nuy.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'galaxstyle':
				...
				ct = body.slice(11)
				reply(ind.wait())
				ct = await getBuffer(`http://lolhuman.herokuapp.com/api/ephoto1/galaxystyle?apikey=682aeab645ed61cf137cf971&text=${ct}`)
				nuy.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'hologram':
				...
				ct = body.slice(9)
				reply(ind.wait())
				ct = await getBuffer(`http://lolhuman.herokuapp.com/api/ephoto1/hologram3d?apikey=682aeab645ed61cf137cf971&text=${ct}`)
				nuy.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'textbyname':
				...
				ct = body.slice(11)
				reply(ind.wait())
				ct = await getBuffer(`http://lolhuman.herokuapp.com/api/ephoto1/textbyname?apikey=682aeab645ed61cf137cf971&text=${ct}`)
				nuy.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'herrypoter':
				...
				ct = body.slice(11)
				reply(ind.wait())
				ct = await getBuffer(`http://lolhuman.herokuapp.com/api/photooxy1/harrypotter?apikey=682aeab645ed61cf137cf971&text=${ct}`)
				nuy.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'greanneon':
				...
				ct = body.slice(10)
				reply(ind.wait())
				ct = await getBuffer(`http://lolhuman.herokuapp.com/api/ephoto1/greenneon?apikey=682aeab645ed61cf137cf971&text=${ct}`)
				nuy.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'metallogo':
				...
				ct = body.slice(10)
				reply(ind.wait())
				ct = await getBuffer(`http://lolhuman.herokuapp.com/api/ephoto1/metallogo?apikey=682aeab645ed61cf137cf971&text=${ct}`)
				nuy.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'bplogo':
				...
				buffer = await getBuffer(anu.result)
				nuy.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'hartatata':
				 // Fix Bug By Nurulツ				
				...
				nuy.sendMessage(from, bh, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'hartatata2':
		...
		case 'attp':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					nuy.sendMessage(from, atytyd, sticker, {quoted: mek})
					await limitAdd(sender) 
					break
		case 'cblackpink':
				 // Fix Bug By Nurulツ				
				...
				bh = body.slice(11)
				reply(ind.wait())
				bh = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome/blackpink?apikey=682aeab645ed61cf137cf971&text=${bh}`)
				nuy.sendMessage(from, bh, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'thunder':
		...
		case 'imgmaker':
				...
				ainez = body.slice(10)
				reply(ind.wait())
				anu = await fetchJson(`http://lolhuman.herokuapp.com/api/quotemaker?apikey=682aeab645ed61cf137cf971&text=${ainez}`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				nuy.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'calendermaker':
				...
				manca = body.slice(15)
				reply(ind.wait())
				anu = await fetchJson(`https://api.zeks.xyz/api/calendar?img=${manca}&apikey=apivinz`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				nuy.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'nekonime':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					reply(ind.wait())
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/nekonime?apikey=Bidmzz1sJ2L1TKyqaMEU`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        nuy.sendMessage(from, buffer, image, {quoted: mek})
					await limitAdd(sender)
                                        break
		case 'husbu':
				...
				reply(ind.wait())
				anu = await fetchJson(`https://tobz-api.herokuapp.com/api/husbu2?apikey=Bidmzz1sJ2L1TKyqaMEU`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				nuy.sendMessage(from, buffer, image, {quoted: mek})
				await limitAdd(sender)
				break
		case 'shota':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				    try {
						res = await fetchJson(`https://tobz-api.herokuapp.com/api/randomshota?apikey=Bidmzz1sJ2L1TKyqaMEU`)
				...
				tels = body.slice(10)
				reply(ind.wait())
				anu = await fetchJson(`https://tobz-api.herokuapp.com/api/textpro?theme=jokerlogo&text=${tels}&apikey=Bidmzz1sJ2L1TKyqaMEU`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				nuy.sendMessage(from, buffer, image, {quoted: mek})
				await limitAdd(sender)
				break
				...
				reply(ind.wait())
				anu = await fetchJson(`https://tobz-api.herokuapp.com/api/randomanime?apikey=Bidmzz1sJ2L1TKyqaMEU`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				nuy.sendMessage(from, buffer, image, {quoted: mek})
				await limitAdd(sender)
				break
				...
				reply(ind.wait())
				buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/toloserti?apikey=682aeab645ed61cf137cf971&name=${gatauda}`, {method: 'get'})
				nuy.sendMessage(from, buffer, image, {quoted: mek})
				await limitAdd(sender)
				break
				case 'emoji2img': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				reply(ind.wait())
				anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/meme?apikey=682aeab645ed61cf137cf971`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				nuy.sendMessage(from, buffer, image, {quoted: mek})
				await limitAdd(sender)
				break
		...
		case 'randomkpop':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				reply(ind.wait())
				anu = await fetchJson(`https://tobz-api.herokuapp.com/api/randomkpop?apikey=Bidmzz1sJ2L1TKyqaMEU`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				nuy.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih kpopnya kak...'})
				await limitAdd(sender)
				break
		case 'aesthetic':
				...
				nuy.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih aestheticnya kak...'})
				await limitAdd(sender)
				break
		case 'gamelogo': 
				gatauda = body.slice(9)
				...
				nuy.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih hasilnya kak...'})
				await limitAdd(sender)
				break
		case 'pornlogo': 
                 if (!isRegistered) return reply( ind.noregis())
				...
				//if (!isPremium) return reply('Maaf kamu bukan user premium!')
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				data = await fetchJson(`https://tobz-api.herokuapp.com/api/joox?q=${body.slice(6)}&apikey=Bidmzz1sJ2L1TKyqaMEU`, {method: 'get'})
				if (data.error) return reply(data.error)
				infomp3 = `*Lagu Ditemukan!!!*\nJudul : ${data.result.judul}\nAlbum : ${data.result.album}\nDipublikasi : ${data.result.dipublikasi}`
				buffer = await getBuffer(data.result.thumb)
				lagu = await getBuffer(data.result.mp3)
				nuy.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
		...
		case 'play':   
				...
				nuy.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: mek})
				await limitAdd(sender)
				break
		case 'limit':
				 // Fix Bug By Nurulツ				
		...
		case 'vinta':
				...
				nuy.sendMessage(from, vintage, image, {caption: 'Nih kak ${vin}', quoted: mek})
				await limitAdd(sender)
				break
		case 'avengers':
				 // Fix Bug By Nurulツ				
				...
				nuy.sendMessage(from, aruga, image, {caption: 'Nih kak', quoted: mek})
				await limitAdd(sender)
				break 
		case 'summer':
				 // Fix Bug By Nurulツ				
		...
		case 'grenneon':
				...
				aruga = body.slice(10)
				reply(ind.wait())
				aruga = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome/greenneon?apikey=682aeab645ed61cf137cf971&text=LoLHuman${aruga}`)
				nuy.sendMessage(from, aruga, image, {caption: 'Nih kak', quoted: mek})
				await limitAdd(sender)
				break 
		case 'neontext':
				...
				aruga = body.slice(10)
				reply(ind.wait())
				aruga = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome/neon?apikey=682aeab645ed61cf137cf971&text=LoLHuman${aruga}`)
				nuy.sendMessage(from, aruga, image, {caption: 'Nih kak', quoted: mek})
				await limitAdd(sender)
				break 
		case 'toxic13':
		...
		case 'firework':
				...
				nuy.sendMessage(from, arugazzz, image, {caption: 'Nih kak', quoted: mek})
				await limitAdd(sender)
				break
		case 'lava':
				 // Fix Bug By Nurulツ				
                ...
                case '1cak':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
						...
						nuy.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih Kak udah Jadi'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(ind.wrongf())
					}
                ...
                case 'faktaunik': // Fixed By Nurulツ & FxSx
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/faktaunik?apikey=682aeab645ed61cf137cf971`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break		
            case 'quotes':
		...
		case 'covidglobal':    
				...
				nuy.updatePresence(from, Presence.composing) 
			    tels = body.slice(12)
				asu = await fetchJson(`http://lolhuman.herokuapp.com/api/corona/global?apikey=682aeab645ed61cf137cf971`)
		...
		case 'ceknamaff': 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					gatauda = body.slice(11)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/freefire/${gatauda}?apikey=682aeab645ed61cf137cf971`)
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'ceknamaml': 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					gatauda = body.slice(11)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/mobilelegend/${gatauda}?apikey=682aeab645ed61cf137cf971`)
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'namaninja': 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					gatauda = body.slice(11)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/ninja?apikey=682aeab645ed61cf137cf971&nama=${gatauda}`)
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'infonomor':
		...
		case 'infogempa':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/infogempa?apikey=Bidmzz1sJ2L1TKyqaMEU`, {method: 'get'})
					if (anu.error) return reply(anu.error)
		...
		case 'infocuaca':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/cuaca?wilayah=${tels}&apikey=Bidmzz1sJ2L1TKyqaMEU`, {method: 'get'})
					if (anu.error) return reply(anu.error)
		...
		case 'github': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					nuy.sendMessage(from, buffer7, image, {quoted: mek, caption: capt})
					await limitAdd(sender)
					break
                case 'slap':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					nuy.sendMessage(from, pod, image, { quoted: mek, caption: '*Toxic*\n\n'+ ple })
					await limitAdd(sender)
					break
		case 'tampar':
					 // Fix Bug By Nurulツ				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					nuy.updatePresence(from, Presence.composing) 
					data = await fetchJson(`http://lolhuman.herokuapp.com/api/newsinfo?apikey=682aeab645ed61cf137cf971`, {method: 'get'})
					teks = '=================\n'
					for (let i of data.result) {
		...
		case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							nuy.sendMessage(_.jid, buffer, image, {caption: `*「 BC GROUP 」*\n\nDari Grup : ${groupName}\nPengirim : wa.me/${(sender.split('@')[0])}\nPesan : ${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
		...
		case 'img':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					nuy.sendMessage(from, pok, image, { quoted: mek, caption: `*Nih Kak*`})
					await limitAdd(sender)
					break 
           ...
           case 'imagemountain':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.shizukaa.xyz/api/wpmountain?apikey=chika633`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					nuy.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
		   ...
		   case 'mimpi':
			...
			case 'katailham':
				...
				kata = anu.result
				nuy.sendMessage(from, kata, text, {quoted: mek })
				await limitAdd(sender)
				break
				case 'caklontong':
			         // Fix Bug By Nurulツ				
                 if (!isRegistered) return reply( ind.noregis())
					...
					setTimeout( () => {
					nuy.sendMessage(from, '*тЮ╕ Jawaban :* '+anu.jawaban+ '\n\nтАв Penjelasan: *'+ anu.keterangan+'*', text, {quoted: mek }) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					nuy.sendMessage(from, '_10 Detik lagiтАж_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					nuy.sendMessage(from, '_20 Detik lagi_тАж', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					nuy.sendMessage(from, '_30 Detik lagi_тАж', text) // ur cods
					}, 2500) // 1000 = 1s,
					setTimeout( () => {
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                 const manca = await nuy.getGroupMembersId(groupId)
                 let gmik = manca[Math.floor(Math.random() * manca.length)]
				...
				case 'tebakgambar':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					setTimeout( () => {
					nuy.sendMessage(from, '*тЮ╕ Jawaban :* '+anu.jawaban, text, {quoted: mek }) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					nuy.sendMessage(from, '_10 Detik lagiтАж_', text) // ur cods
					}, 20000) // 1000 = 1s,
		...
		case 'spamcall':
			...
			case 'ytmp3':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					reply(ind.wait())
					buffer = await getBuffer(anu.result)
					nuy.sendMessage(from, buff, image, {quoted: mek, caption: teks})
					nuy.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek })
					await limitAdd(sender) 
					break
			 case 'nsfwloli':
				    try {
		...
		case 'lirik': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					teks = body.slice(6)
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/lirik?q=${teks}&apikey=Bidmzz1sJ2L1TKyqaMEU`, {method: 'get'})
		...
		case 'alay2': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply('kasih teks lah^_^!!!')
					data = await fetchJson(`http://lolhuman.herokuapp.com/api/upperlower?apikey=682aeab645ed61cf137cf971&text=${body.slice(6)}`)
					reply(data.result)
				        await limitAdd(sender)
					break
		case 'fml':	// Fixed By Nurulツ & FxSx
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					data = await fetchJson(`https://tobz-api.herokuapp.com/api/randomfmylife?apikey=Bidmzz1sJ2L1TKyqaMEU`)
					hasil = data.result
					reply(hasil)
					await limitAdd(sender)
					break	
		case 'chord':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					tels = body.slice(7)					
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/chord?q=${tels}&apikey=Bidmzz1sJ2L1TKyqaMEU`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'katacinta':
		...
		case 'cerpen': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/cerpen?apikey=682aeab645ed61cf137cf971`, {method: 'get'})
		...
		case 'infofilm': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					if (isBanned) return reply('Maaf kamu sudah terbenned!')
					ige = body.slice(10)
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/film2?q=${ige}&apikey=Bidmzz1sJ2L1TKyqaMEU`)
		...
		case 'infofilm2': 
                 if (!isRegistered) return reply( ind.noregis())
					...
					if (isBanned) return reply('Maaf kamu sudah terbenned!')
					ige = body.slice(10)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/lk21?apikey=682aeab645ed61cf137cf971&query=${ige}`)
					buffer7 = await getBuffer(anu.result.thumbnail)
		...
		case 'heroml':  
				...
				nuy.sendMessage(from, buffer7, image, {quoted: mek, caption: teks})
				await limitAdd(sender)
				break
				case 'igvideo': 
                 if (!isRegistered) return reply( ind.noregis())
				...
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
					 ige = body.slice(9)
                     anu = await fetchJson(`http://lolhuman.herokuapp.com/api/instagram?apikey=682aeab645ed61cf137cf971&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
				...
				if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(mess.error.lv)
					 ige = body.slice(12)
                     anu = await fetchJson(`http://lolhuman.herokuapp.com/api/tiktok?apikey=682aeab645ed61cf137cf971&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
		...
		case 'grouplist':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					nuy.sendMessage(from, teks.trim(), extendedText, {quoted: mek})
					break
		//daftar 
		case 'daftar':
                			...
                			if (umurUser < 12) return reply(`your age is too young minimum 12 years`)
                    			...
                    			await nuy.sendMessage(from, ppimg, image, {quoted: mek, caption: ind.registered(namaUser, umurUser, serialUser, time, sender)})
                    			addATM(sender)
		...
		case 'ping3':
					 // Fix Bug By Nurulツ				
                 if (!isRegistered) return reply( ind.noregis())
					await nuy.sendMessage(from, `Pong!!!!\nSpeed: ${processTime(time, moment())} _Second_`)
					break
		case 'help': 
		case 'menu':
              if (!isRegistered) return reply( ind.noregis())
...
nuy.sendMessage(from, menu, image, {quoted: fxsx, caption: faa})
		    break
		case 'donasi':
		case 'donate':
					 // Fix Bug By Nurulツ				
                 if (!isRegistered) return reply( ind.noregis())
					nuy.sendMessage(from, donasi(), text)
					break
		case 'menupicture':
		case 'menupict':
		case 'pictmenu':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					nuy.sendMessage(from, wew, image, { quoted: fxsx, caption: randompict(prefix)})
					break
		case 'menuowner':
		case 'ownermenu':
					 // Fix Bug By Nurulツ				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					wew = fs.readFileSync(`./src/logo.jpg`)
					nuy.sendMessage(from, wew, image, { quoted: fxsx, caption: developer(prefix)})
					break
		case 'bahasa':
					 // Fix Bug By Nurulツ				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					nuy.sendMessage(from, bahasa(), text)
					await limitAdd(sender)
					break
		case 'kodenegara':
					 // Fix Bug By Nurulツ				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					nuy.sendMessage(from, negara(), text)
					await limitAdd(sender)
					break
		case 'del':
		case 'd':
		...
		case 'level':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					nuy.sendMessage(from, resul, text, { quoted: mek})
					.catch(async (err) => {
					console.error(err)
					await reply(`Error!\n${err}`)
					})
		...
		case 'say':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        ...
                                        saying = teks
                                        nuy.sendMessage(from, saying, text)
					await limitAdd(sender)
                                        break
		case 'info':
					me = nuy.user
		...
		case 'lpr':
					...
					if (pesan.length > 300) return nuy.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
		...
		case 'request':
					...
					if (cfrr.length > 300) return nuy.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
					var nomor = mek.participant
		...
		case 'blocklist': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                                ...
                                const tagzz = {
                                        ...
                                        nuy.sendMessage(from, tagzz, text, {quoted: mek})
					await limitAdd(sender)
                                        break
                                        case 'profile':
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					nuy.sendMessage(from, buffer, image, {quoted: mek, caption: profile})
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					buffer = await getBuffer(anu.result)
					nuy.sendMessage(from, buffer, image, {caption: 'Nih kak', quoted: mek})
					await limitAdd(sender)
					break
                case 'truth':
					 // Fix Bug By Nurulツ				
		...
		case 'quoteskehidupan':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					nuy.sendMessage(from, crot, image, { quoted: mek, caption: '*Quotes Kehidupan*\n\n'+ quo })
					await limitAdd(sender)
					break
		case 'quotesislami':
					 // Fix Bug By Nurulツ				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					nuy.sendMessage(from, islam, image, { quoted: mek, caption: '*Quotes Islami*\n\n'+ isl })
					await limitAdd(sender)
					break	
		case 'quotesnasehat':
					 // Fix Bug By Nurulツ				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					nuy.sendMessage(from, nase, image, { quoted: mek, caption: '*Quotes Nasehat*\n\n'+ nsh })
					await limitAdd(sender)
					break	
		case 'quotescinta':
					 // Fix Bug By Nurulツ				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					nuy.sendMessage(from, cta, image, { quoted: mek, caption: '*Quotes Cinta*\n\n'+ cin })
					await limitAdd(sender)
					break
		case 'quotesmotivasi':
					 // Fix Bug By Nurulツ				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					nuy.sendMessage(from, vass, image, { quoted: mek, caption: '*Motivasi*\n\n'+ cin })
					await limitAdd(sender)
					break
                case 'fotocewek':
					 // Fix Bug By Nurulツ				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
	                                const cangti = ['https://i.ibb.co/1T1DCz7/cewek-thailand-20200325-007-non-fotografer-kly.jpg','https://i.ibb.co/FsJ6jjs/kucing2.jpg','https://i.ibb.co/vvkdS7n/kucing3.jpg','https://i.ibb.co/1QHWxts/kucing4.jpg','https://i.ibb.co/JQmRz4n/kucing5.jpg','https://i.ibb.co/tBwrFkG/kucing6.jpg','https://i.ibb.co/dp0YhYm/kucing7.jpg','https://i.ibb.co/R03smZT/kucing8.jpg','https://i.ibb.co/17tw0dp/kucing9.jpg','https://i.ibb.co/7XdGGqc/kucing10.jpg','https://i.ibb.co/XL9PZxg/kucing11.jpg','https://i.ibb.co/gyjvXWN/kucing12.jpg','https://i.ibb.co/R4gg4wH/kucing13.jpg','https://i.ibb.co/PmLYtFm/kucing14.jpg','https://i.ibb.co/XbSDh47/kucing15.jpg','https://i.ibb.co/kSXNJzt/kucing16.jpg']
                                        ...
                                        nuy.sendMessage(from, cangtip, image, {quote: mek})
					await limitAdd(sender)
                                        break
		case 'ssweb':
					 // Fix Bug By Nurulツ				
		...
		case 'loli':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                                        gatauda = body.slice(6)
					reply(ind.wait())
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/randomloli?apikey=Bidmzz1sJ2L1TKyqaMEU`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        nuy.sendMessage(from, buffer, image, {quoted: mek})
					await limitAdd(sender)
                                        break
		case 'waifu':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					nuy.sendMessage(from, hasil, image, {quoted: mek})
					await limitAdd(sender)
					break
		case 'loli2':
					 // Fix Bug By Nurulツ				
		...
		case 'animecry':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson('https://tobz-api.herokuapp.com/api/cry?apikey=Bidmzz1sJ2L1TKyqaMEU', {method: 'get'})
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
		...
		case 'animehug':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson('https://tobz-api.herokuapp.com/api/hug?apikey=Bidmzz1sJ2L1TKyqaMEU', {method: 'get'})
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(mess.error.stick)
		...
		case 'map':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					buffer = await getBuffer(anu.gambar)
					nuy.sendMessage(from, buffer, image, {quoted: mek, caption: `${body.slice(5)}`})
					await limitAdd(sender)
					break
                case 'kbbi':
					 // Fix Bug By Nurulツ				
		...
		case 'jadwalsholat':
				...
				if (args.length < 1) return reply('kota nya mana kak?')
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/jadwalshalat?q=${body.slice(14)}&apikey=Bidmzz1sJ2L1TKyqaMEU`, {method: 'get'})
					sholat = `Ashar : ${anu.result.ashar}\nDzuhur : ${anu.result.dzuhur}\nMagrib : ${anu.result.maghrib}\nIsha : ${anu.result.isha}\nSubuh : ${anu.result.subuh}`
					nuy.sendMessage(from, sholat, text, {quoted: mek})
					await limitAdd(sender)
					break
                case 'neonime':
				         // Fix Bug By Nurulツ				
		...
		case 'tts':
				...
				//if (!isPremium) return reply('Maaf kamu bukan user premium!')
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (args.length < 1) return nuy.sendMessage(from, 'Diperlukan kode bahasa kak!!', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return nuy.sendMessage(from, 'Mana teks yang mau di jadiin suara? suara setan kah?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('Textnya kebanyaka!! 😤')
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							fs.unlinkSync(ranm)
							buffer = fs.readFileSync(rano)
							if (err) return reply(ind.stikga())
							nuy.sendMessage(from, buffer, audio, {quoted: mek, ptt:true})
							fs.unlinkSync(rano)
						})
					})
					await limitAdd(sender)
		...
		case 'tiktokstalk':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				try {
						if (args.length < 1) return nuy.sendMessage(from, '*Username mana kak?', text, {quoted: mek})
		...
		case 'unblock':
					...
					nuy.sendMessage(from, `*Perintah Diterima, Membuka Blockir* ${body.slice(9)}@c.us`, text)
					break
		case 'leave':
					 // Fix Bug By Nurulツ				
                 if (!isRegistered) return reply( ind.noregis())
					...
					setTimeout( () => {
					nuy.updatePresence(from, Presence.composing) 
					nuy.sendMessage(from, 'Sampai Jumpa Semuanya👋', text) // ur cods
					}, 0)
					break
		case 'bc': 
					if (!isOwner) return reply(ind.ownerb()) 
		...
		case 'group':
            ...
            case 'developer':
                  ...
                  nuy.sendMessage(from, 'Tuh nomer owner ku >_<, jangan spam atau ku block kamu',MessageType.text, { quoted: mek} )
					break    
		case 'kick':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
		...
		case 'toimg':
				...
				if (!isQuotedSticker) return reply('*Reply/Tag sticker!*')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						nuy.sendMessage(from, buffer, image, {quoted: mek, caption: '*DAH JADI SETAN* '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
		...
		case 'welcome':
                 ...
                 case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						nuy.sendMessage(from,`Perhatian kepada seluruh member anti link group aktif apabila anda mengirim link group anda akan di kick dari group`, text)
					} else if (Number(args[0]) === 0) {
